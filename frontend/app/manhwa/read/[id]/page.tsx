...
import { ArrowLeft, ArrowRight, ChevronLeft, ChevronRight, Loader2, AlertTriangle, BookOpen } from "lucide-react";
import { api, type ChapterInfo, type ManhwaItem } from "@/lib/api";
import { needsUnoptimized } from "@/lib/utils";

export default function ManhwaReaderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then((p) => setChapterId(p.id));
  }, [params]);
